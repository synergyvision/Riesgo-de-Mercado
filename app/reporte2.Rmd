...
output: 
  pdf_document:
    includes:
      in_header: C:/Users/Freddy Tapia/Documents/preambulo.tex
...
```{r,echo = FALSE}
kable(cbind.data.frame(c("B0","B1","B2","T1"),params$par_ns_v),col.names=c("Parámetros","Valores"),align="c")
```

Los parámetros optimizados obtenidos para la metodología de Svensson son,

```{r,echo = FALSE}
kable(cbind.data.frame(c("B0","B1","B2","B3","T1","T2"),params$par_sven_v),col.names=c("Parámetros","Valores"),align="c")
```
...
library(knitr)
kable(params$comp_pre_v,align="c")
```
...
library(plotly)
    ...
    x <- seq(1,20,0.1)
    y_ns <- nelson_siegel(x,pa=params$par_ns_v)*100
    y_sven <- sven(x,pa=params$par_sven_v)*100
    y_dl <- diebold_li(pa=as.numeric(params$par_dl_v),x)*100
    y_sp <- predict(params$par_sp_v,x*365)$y
