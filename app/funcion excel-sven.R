#funcion Excel-Svensson DEFINITIVA
#ARGUMENTOS
#fv: fecha de valoraci贸n ej: "11/08/2017"
#tit: titulos a considerar (por ahora solo TIF)
#pr: precios promedio de los titulos-OJO-los primeros 4 se pueden ajustar 
#manualmente
#pa: parametros iniciales
#ind: 0 = Tif o 1 = veb
...
#ARGUMENTOS
#tit: t铆tulo o t铆tulos a considerar, debe ser el nombre corto
...
#que se asemejen los m谩s posible a los precios promedio ingresados
#ARGUMENTOS
#fv: fecha de valoraci贸n, ej: "11/08/2017"
#tit: t铆tulo o t铆tulos a considerar, debe ser el nombre corto
#ej: TIF082018 (OJO, mejor considerar el ISIN)
...
Tabla.sven=function(fv,tit,pr,pa,ind,C){
  ...
  colnames(Tabla)=tit
  rownames(Tabla)=c("ISIN","Fecha de Liquidaci贸n",
                    "Fecha de emisi贸n","Fecha de Vencimiento","Tasa de Cup贸n",
                    "Precio Prom","Fecha 煤ltimo Pago","Fecha pr贸ximo pago",
                    "RDTO al VMTO","Duraci贸n","Inverso de la duraci贸n",
                    "Ponderaci贸n","Precio Modelo Svensson Ajustado",
  ...
  #a帽ado precios promedios
  Tabla[6,]=pr
  
  if(ind==0){
  #a帽ado rendimiento al vencimiento y duracion
  ...
  if(fe2==1){
    ...
    if(fe3==1){
  #Broyden鈥揊letcher鈥揋oldfarb鈥揝hanno-metodo cuasi Newton
  ...
  if(q==q1){
    print("OPtimizaci髇 fallida..")
    print("Optimizando por un m閠odo diferente")
    ala1=nloptr::auglag(pa, fn=mifuncion, hin=res)
    ala<<-ala1
    Tabla[13,]=precio.sven(tit,fv,C,ala1$par)
  }
  ...
  if(fe2==0){
    print("No se  optimizar谩, se mantendr谩n los mismos precios")
...
#pa=c(0.133799434790145,-0.0379321105061609,-0.307885339616438,-0.134075672659356,
...
#vebono
Tabla11v=Tabla.sven(fv = "06/07/2018",tit = tit1,pr = pr1,pa = pa1,ind = 1,C = C) 
