  ...
  if(ind==0){
    ...
    while(length(which(Tabla[9,]<0))!=0){
      ...
      for(i in 1:length(which(Tabla[9,]<0))){
        ...
        aa <- find_pre(as.numeric(gsub("[,]",".",Tabla[6,i])),Tabla)
        
        if(class(aa)=="numeric"){
        vt[i] <- aa
        }else{
          return("Error de precio promedio")
        }
      }
...
#PRUEBA CASO VEBONO VEB012020 Y VEB062020
Tabla.ns=function(fv,tit,pr,pa,ind,C,fe2,fe3){
  ...
  if(ind==1){
    ...
    while(length(which(Tabla[9,]<0))!=0){
      ...
      for(i in 1:length(which(Tabla[9,]<0))){
        #vt[i] <- as.numeric(readline(prompt="Ej: 101.05,  "))
        aa <- find_pre(as.numeric(gsub("[,]",".",Tabla[6,rendneg[i]])),Tabla,fv,rendneg[i])
        
        if(class(aa)=="numeric"){
          vt[i] <- aa
        }else{
          return("Error con precio promedio")
        }
...
tit1[c(6,7)]
pos1(t = tit1[c(6,7)],1,Precio_prom_veb)

#se queda pegado
p1 <- Tabla.ns(fv="03/05/2019",tit1[c(6,7)],pos1(t = tit1[c(6,7)],1,Precio_prom_veb),pa=pa_ns,ind=1,C=ca1,fe2=0,fe3=0)

#prueba
p2 <- Tabla.ns(fv="03/05/2019",tit1[c(6,7)],c(115,125),pa=pa_ns,ind=1,C=ca1,fe2=0,fe3=0)

#prueba solo find_pre (funcion mas interna)
fv="03/05/2019"
tit=tit1[c(6,7)]
pr=pos1(t = tit1[c(6,7)],1,Precio_prom_veb)
pa=pa_ns
ind= 1
C= ca1
fe2=0
...
find_pre(pre = ,Tabla = ,fv = ,i = )

pre= as.numeric(gsub("[,]",".",Tabla[6,rendneg[i]]))
i <- 2
